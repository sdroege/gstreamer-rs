...
use crate::{
    ffi, RTSPAuth, RTSPContext, RTSPFilterResult, RTSPMountPoints, RTSPSession, RTSPSessionPool,
    RTSPThreadPool,
};
use glib::{
...
pub trait RTSPClientExt: IsA<RTSPClient> + 'static {
    ...
    #[doc(alias = "pre-closed")]
    fn connect_pre_closed<F: Fn(&Self) + Send + Sync + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn pre_closed_trampoline<
            P: IsA<RTSPClient>,
            F: Fn(&P) + Send + Sync + 'static,
        >(
            this: *mut ffi::GstRTSPClient,
