...
use crate::{ChildProxy, Element, ElementFlags, Object, Pad, PadDirection, ffi};
use glib::{
...
pub trait GstBinExt: IsA<Bin> + 'static {
    ...
    fn connect_async_handling_notify<F: Fn(&Self) + Send + Sync + 'static>(
        ...
        unsafe {
            ...
            connect_raw(
                self.as_ptr() as *mut _,
                c"notify::async-handling".as_ptr(),
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(
    ...
    fn connect_message_forward_notify<F: Fn(&Self) + Send + Sync + 'static>(
        ...
        unsafe {
            ...
            connect_raw(
                self.as_ptr() as *mut _,
                c"notify::message-forward".as_ptr(),
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(
