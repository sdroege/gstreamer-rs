...
pub trait DiscovererExt {
    ...
    fn discover_uri_async(&self, uri: &str) -> Result<(), glib::error::BoolError>;

    fn start(&self);

    fn stop(&self);

    fn connect_discovered<F: Fn(&Self, &DiscovererInfo, &Option<Error>) + Send + Sync + 'static>(&self, f: F) -> SignalHandlerId;
...
impl<O: IsA<Discoverer> + IsA<glib::object::Object>> DiscovererExt for O {
    ...
    fn connect_discovered<F: Fn(&Self, &DiscovererInfo, &Option<Error>) + Send + Sync + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe {
            let f: Box_<Box_<Fn(&Self, &DiscovererInfo, &Option<Error>) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
...
where P: IsA<Discoverer> {
    callback_guard!();
    let f: &&(Fn(&P, &DiscovererInfo, &Option<Error>) + Send + Sync + 'static) = transmute(f);
