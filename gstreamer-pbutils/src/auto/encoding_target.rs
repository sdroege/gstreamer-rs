...
impl EncodingTarget {
    ...
    pub fn load_from_file(filepath: &str) -> Result<EncodingTarget, Error> {
        ...
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::gst_encoding_target_load_from_file(filepath.to_glib_none().0, &mut error);
            if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
        }
    }
}
...
pub trait EncodingTargetExt {
    ...
    fn save_to_file(&self, filepath: &str) -> Result<(), Error>;
}

impl<O: IsA<EncodingTarget>> EncodingTargetExt for O {
    fn add_profile<P: IsA<EncodingProfile>>(&self, profile: &P) -> bool {
        unsafe {
            from_glib(ffi::gst_encoding_target_add_profile(self.to_glib_none().0, profile.to_glib_full()))
        }
    }
    ...
    fn save_to_file(&self, filepath: &str) -> Result<(), Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let _ = ffi::gst_encoding_target_save_to_file(self.to_glib_none().0, filepath.to_glib_none().0, &mut error);
            if error.is_null() { Ok(()) } else { Err(from_glib_full(error)) }
        }
    }
}
