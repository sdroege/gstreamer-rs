...
unsafe extern "C" fn base_parse_sink_query<T: BaseParseImpl>(
    ...
    unsafe {
        ...
        gst::panic_to_error!(imp, false, { imp.sink_query(query) }).into_glib()
    }
}
...
unsafe extern "C" fn base_parse_src_query<T: BaseParseImpl>(
    ...
    unsafe {
        ...
        gst::panic_to_error!(imp, false, { imp.src_query(query) }).into_glib()
    }
}
...
unsafe extern "C" fn base_parse_detect<T: BaseParseImpl>(
    ...
    unsafe {
        ...
        let res = gst::panic_to_error!(imp, Err(gst::FlowError::Error), { imp.detect(buffer) });
