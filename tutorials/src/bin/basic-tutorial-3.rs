...
fn main() {
    ...
    source.connect_pad_added(move |_, src_pad| {
        ...
        let sink_pad = convert.get_static_pad("sink")
        ...
        let new_pad_caps = src_pad.get_current_caps()
            .expect("Failed to get caps of new pad.");
        let new_pad_struct = new_pad_caps.get_structure(0)
            .expect("Failed to get first structure of caps.");
        let new_pad_type = new_pad_struct.get_name();

        let is_audio = new_pad_type.starts_with("audio/x-raw");
        if !is_audio {
            println!("It has type {} which is not raw audio. Ignoring.",
                     new_pad_type);
            return
        }
