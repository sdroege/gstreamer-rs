...
fn print_pad_template_information(factory: &gst::ElementFactory) {
    let long_name = factory.get_metadata("long-name")
    ...
    match pad.get_current_caps() {
      ...
      None => {
          let caps = pad.query_caps(None)
              .expect("Failed to query caps on pad");
          print_caps(caps, "      ");
      }
    }
}
...
fn main() {
    ...
    let source_factory = gst::ElementFactory::find("audiotestsrc")
        .expect("Failed to create audiotestsrc factory.");
    ...
    let source = source_factory.create("source")
        .expect("Failed to create source element");
    let sink = sink_factory.create("sink")
    ...
    if ret == gst::StateChangeReturn::Failure {
        eprintln!("Unable to set the pipeline to the playing state (check the bus for error messages).")
    }
