...
impl Player {
    ...
    pub fn connect_buffering<F: Fn(&Player, i32) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, i32) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "buffering",
                transmute(buffering_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_duration_changed<F: Fn(&Player, u64) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
    ...
    pub fn connect_end_of_stream<F: Fn(&Player) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
    ...
    pub fn connect_error<F: Fn(&Player, &Error) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, &Error) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "error",
                transmute(error_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_media_info_updated<F: Fn(&Player, &PlayerMediaInfo) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, &PlayerMediaInfo) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "media-info-updated",
                transmute(media_info_updated_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_mute_changed<F: Fn(&Player) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
    ...
    pub fn connect_seek_done<F: Fn(&Player, u64) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, u64) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
    ...
    pub fn connect_state_changed<F: Fn(&Player, PlayerState) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, PlayerState) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "state-changed",
                transmute(state_changed_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_uri_loaded<F: Fn(&Player, &str) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, &str) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "uri-loaded",
                transmute(uri_loaded_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_video_dimensions_changed<F: Fn(&Player, i32, i32) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, i32, i32) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
    ...
    pub fn connect_volume_changed<F: Fn(&Player) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
    ...
    pub fn connect_warning<F: Fn(&Player, &Error) + Send + Sync + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Player, &Error) + Send + Sync + 'static>> = Box_::new(Box_::new(f));
...
unsafe extern "C" fn buffering_trampoline(this: *mut ffi::GstPlayer, object: libc::c_int, f: glib_ffi::gpointer) {
    callback_guard!();
    let f: &&(Fn(&Player, i32) + Send + Sync + 'static) = transmute(f);
...
unsafe extern "C" fn error_trampoline(this: *mut ffi::GstPlayer, object: *mut glib_ffi::GError, f: glib_ffi::gpointer) {
    callback_guard!();
    let f: &&(Fn(&Player, &Error) + Send + Sync + 'static) = transmute(f);
...
unsafe extern "C" fn media_info_updated_trampoline(this: *mut ffi::GstPlayer, object: *mut ffi::GstPlayerMediaInfo, f: glib_ffi::gpointer) {
    callback_guard!();
    let f: &&(Fn(&Player, &PlayerMediaInfo) + Send + Sync + 'static) = transmute(f);
    f(&from_glib_none(this), &from_glib_none(object))
}
...
unsafe extern "C" fn state_changed_trampoline(this: *mut ffi::GstPlayer, object: ffi::GstPlayerState, f: glib_ffi::gpointer) {
    callback_guard!();
    let f: &&(Fn(&Player, PlayerState) + Send + Sync + 'static) = transmute(f);
...
unsafe extern "C" fn uri_loaded_trampoline(this: *mut ffi::GstPlayer, object: *mut libc::c_char, f: glib_ffi::gpointer) {
    callback_guard!();
    let f: &&(Fn(&Player, &str) + Send + Sync + 'static) = transmute(f);
...
unsafe extern "C" fn video_dimensions_changed_trampoline(this: *mut ffi::GstPlayer, object: libc::c_int, p0: libc::c_int, f: glib_ffi::gpointer) {
    callback_guard!();
    let f: &&(Fn(&Player, i32, i32) + Send + Sync + 'static) = transmute(f);
